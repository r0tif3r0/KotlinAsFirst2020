...
fun containsIn(a: Map<String, String>, b: Map<String, String>): Boolean {
    for ((key, value) in a) {
        if (key !in b || value != b.getValue(key)) return false
    }
    return true
}
...
fun subtractOf(a: MutableMap<String, String>, b: Map<String, String>): Map<String, String> {
    for ((key, value) in b) {
        if (key in a && value == a.getValue(key))
            a.remove(key)
    }
    return a
}
...
fun averageStockPrice(stockPrices: List<Pair<String, Double>>): Map<String, Double> {
    val avr = mutableMapOf<String, Double>()
    val count = mutableMapOf<String, Int>()
    for ((a, s) in stockPrices) {
        avr[a] = ((avr[a] ?: 0.0) * (count[a] ?: 0) + s) / ((count[a] ?: 0) + 1)
...
fun findSumOfTwo(list: List<Int>, number: Int): Pair<Int, Int> {
    ...
    for (i in list.indices) {
        ...
        if (ind.containsKey(tmp) && i != ind[tmp])
            return Pair(i, ind[tmp]!!)
    }
    return Pair(-1, -1)
}
